...
class Tools {
  ...
  static String getFormattedDateOnly(int dateTime, Locale _locale) {
    DateFormat newFormat = DateFormat("dd MMMM yyyy", _locale.languageCode);
    return newFormat.format(DateTime.fromMillisecondsSinceEpoch(dateTime));
  }
  ...
  static String formatTimeRemoveSeconds(String? s) {
    if(s == null) return "";
    //split string based on :
    var parts = s.split(":");
    //get hours and minutes
    ...
    var minutes = int.parse(parts[1]);
    //convert to 12 hour format in the form of HH:MM AM/PM
    var time = "${(hours > 12) ? hours - 12 : hours}:${(minutes < 10) ? '0$minutes' : minutes}";
    return time;
  }
